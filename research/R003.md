...
Token burning involves permanently removing tokens from circulation, typically done by sending tokens to a burn address. A burn address is simply a wallet that cannot be accessed because there is not an associated private key.

There are several viable methods for burning tokens. The [0x0000](https://etherscan.io/address/0x0000000000000000000000000000000000000000)… address is the most commonly accepted address for burning. However, as defined in the ERC20 implementation, tokens must have the _burn() function to be sent to this address and successfully burned.
...
Scenario: Alice wants to burn 1000 tokens each week from her DAO’s address. To do so she calls _burn(“0xDAO”, 1000).

Requirements:
- _account_ cannot be the zero address
- _account_ must have at least amount tokens
Advantages:
- _burn_ allows users to send tokens to the ‘canonical’ burn address: 0x0000…, effectively removing their tokens from total supply
